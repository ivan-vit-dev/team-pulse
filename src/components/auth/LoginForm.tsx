...
import { signInWithEmail } from "@/lib/auth/client-actions";
...
export function LoginForm({ redirectTo }: { redirectTo: string }) {
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      await signInWithEmail(values.email, values.password, locale);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <Button type="submit" disabled={isSubmitting} className="w-full gradient-brand">
        {isSubmitting ? t("common.saving") : t("auth.loginButton")}
      </Button>
    </form>
  );
}
